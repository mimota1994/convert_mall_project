...
public class UserController {
    ...
    @ResponseBody
    public ServerResponse<User> update_information(HttpSession session, @RequestBody Map<String, String> map){
        User currentUser = (User)session.getAttribute(Const.CURRENT_USER);
        if(currentUser == null){
            return ServerResponse.createByErrorMessage("用户未登录");
        }

        ServerResponse<User> response = iUserService.updateInformation(currentUser, map);
        if(response.isSuccess()){
